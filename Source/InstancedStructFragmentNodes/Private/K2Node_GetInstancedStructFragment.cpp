...
#include "K2Node_GetInstancedStructFragment.h"
...
FInstancedStruct UK2Node_GetInstancedStructFragment::FindFragmentByClass(
	const FInstancedFragmentContainer& InContainer,
	const UScriptStruct* StructType,
	bool& bValid)
{
	return InContainer.FindFragmentByClass(StructType, bValid);
}

void UK2Node_GetInstancedStructFragment::AllocateDefaultPins()
{

	CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Exec, UEdGraphSchema_K2::PN_Execute);
	CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Struct, FInstancedFragmentContainer::StaticStruct(), UK2Node_InstancedStructQuery_PinNames::TargetPinName);
...
void UK2Node_GetInstancedStructFragment::ExpandNode(FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{

	Super::ExpandNode(CompilerContext, SourceGraph);

	UFunction* GetStructPropertyFunc = StaticClass()->FindFunctionByName(GET_FUNCTION_NAME_CHECKED(ThisClass, FindFragmentByClass));
	...
	if (!GetStructPropertyFunc || !GetInstStructValueFunc)
	{
		CompilerContext.MessageLog.Error(*LOCTEXT("InvalidClass", "The FindFragmentByClass or GetInstancedStructValue functions have not been found. Check ExpandNode in K2Node_GetInstancedStructFragment.cpp").ToString(), this);
		return;
	}
	...
	GetPropertyTypeNode->SetFromFunction(GetStructPropertyFunc);
	...
	//----------------------------------------------------------------------------------------
	UEdGraphPin* InContainerPin = GetPropertyTypeNode->FindPinChecked(TEXT("InContainer"));
	...
	bIsErrorFree &= CompilerContext.MovePinLinksToIntermediate(*GetTargetPin(), *InContainerPin).CanSafeConnect();
...
void UK2Node_GetInstancedStructFragment::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
FText UK2Node_GetInstancedStructFragment::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
  return LOCTEXT("UK2Node_GetInstancedStructFragment_NodeTitle", "Get Fragment Data In Container");
}

FText UK2Node_GetInstancedStructFragment::GetTooltipText() const
{
  return LOCTEXT("UK2Node_GetInstancedStructFragment_TooltipText", "Outputs the item fragment based on the fragment container and selected struct type");
}

FText UK2Node_GetInstancedStructFragment::GetKeywords() const
{
  return LOCTEXT("GetInstancedStructFragment", "Get Fragment Data In Container");
}